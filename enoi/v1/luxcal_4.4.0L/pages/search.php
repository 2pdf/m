...
if (!$set['xField1Public'] and $_SESSION['uid'] == 1) { $set['evtTemplGen'] = str_replace('4','',$set['evtTemplGen']); } //exclude xField 1
if (!$set['xField2Public'] and $_SESSION['uid'] == 1) { $set['evtTemplGen'] = str_replace('5','',$set['evtTemplGen']); } //exclude xField 2
...
function catList($selCat) {
	...
	while (list($ID,$name,$color,$bgColor) = $stH->fetch(PDO::FETCH_NUM)) {
		$selected = ($selCat == $name) ? " selected='selected'" : '';
...
function searchForm() {
	global $xx, $set, $schText, $eF, $grpName, $catName, $fromDda, $tillDda;
	...
	foreach (str_split($set['evtTemplGen']) as $fieldNr) {
		if (strpos('1345',$fieldNr) !== false) {
			switch ($fieldNr) {
			case '1': 
				echo "<tr>\n<td></td><td><input type='checkbox' id='eF2' name='eF[]' value='2' onclick=\"checkN('eF',this);\"".(in_array(2, $eF) ? " checked='checked'" : '')."> <label for='eF2'>{$xx['sch_venue']}</label></td>\n</tr>\n";
				break;
			case '3':
				echo "<tr>\n<td></td><td><input type='checkbox' id='eF3' name='eF[]' value='3' onclick=\"checkN('eF',this);\"".(in_array(3, $eF) ? " checked='checked'" : '')."> <label for='eF3'>{$xx['sch_description']}</label></td>\n</tr>\n";
				break;
			case '4':
				echo "<tr>\n<td></td><td><input type='checkbox' id='eF4' name='eF[]' value='4' onclick=\"checkN('eF',this);\"".(in_array(4, $eF) ? " checked='checked'" : '')."> <label for='eF4'>".($set['xField1Label'] ? "{$set['xField1Label']}" : $xx['sch_extra_field1'])."</label></td>\n</tr>\n";
				break;
			case '5':
				echo "<tr>\n<td></td><td><input type='checkbox' id='eF5' name='eF[]' value='5' onclick=\"checkN('eF',this);\"".(in_array(5, $eF) ? " checked='checked'" : '')."> <label for='eF5'>".($set['xField2Label'] ? "{$set['xField2Label']}" : $xx['sch_extra_field2'])."</label></td>\n</tr>\n";
			}
		}
	}
...
function validateForm() {
	...
	if ($schText and strlen(str_replace('_', '', $schText)) < 1) { return $xx['sch_invalid_search_text']; }
...
function searchText() {
	...
	$filter = '';
	if ($grpName != '*') { $filter .= " AND g.`name` = '{$grpName}'"; }
...
function showMatches() {
	...
	if ($evtList) {
		$schMatch = $schText ? $schText : '```'; //```: dummy search match
		$match = '%('.str_replace(array('_','&'), array('.','[^<>]+?'), $schMatch).')(?![^<]*>)%i'; //convert to regex (?!: neg.look-ahead condition)
		$evtDone = array();
		$lastDate = '';
			...
			foreach ($events as $evt) {
				if (!$evt['mde'] or !in_array($evt['eid'],$evtDone)) { //!mde or mde not processed
					...
					if ($evt['cbx']) {
						...
						$cBoxAtt = ($mayCheck and !$evt['mde']) ? " class='chkBox point' onclick=\"checkE(this,{$evt['eid']},'{$date}');\"" : 'class="chkBox arrow"';
					...
					if ($set['details4All'] == 1 or ($set['details4All'] == 2 and $_SESSION['uid'] > 1) or $evt['mayE']) {
						echo "<h6>{$chBox}<span class='point'{$eStyle} onclick=\"editE({$evt['eid']},'{$date}');\">".$eTitle."</span></h6>\n";
						echo preg_replace($match, '<mark>$1</mark>',makeE($evt,$set['evtTemplGen'],'br',"<br>\n"))."\n";
					} else {
						echo "<h6>{$chBox}<span{$eStyle}>".$eTitle."</span></h6>\n";
					}
					echo "</td></tr></table>\n";
					$lastDate = $evtDate;
				}
