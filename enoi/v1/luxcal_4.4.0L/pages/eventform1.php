...
echo "<form id='event' action='index.php?lc&amp;xP=10' name='event' method='post' enctype='multipart/form-data'>
...
echo "<table class='evtForm'>
	...
	$totEvtTempl = $set['evtTemplGen'].$set['evtTemplUpc'].$set['evtTemplPop'];
...
if ($att) {
	...
	foreach(explode(';',trim($att,';')) as $attachment) {
		echo "<tr><td>{$label}</td><td class='attachment point' colspan='3' title='{$xx['evt_click_to_remove']}' onclick=\"detach(this,{$eid},'{$attachment}');\">".substr($attachment,14)."</td></tr>\n";
		$label = '';
	}
}
if ($uplod) { //may upload attachments
	echo "<tr><td>{$xx['evt_attach_file']}</td><td colspan='3'><input type='file' id='upload' name='upload[]' multiple></td></tr>";
}
