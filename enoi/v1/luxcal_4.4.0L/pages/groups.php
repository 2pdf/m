...
function showGroups() {
	...
	echo "<table class='list'>
		<tr><th>&nbsp;{$ax['id']}&nbsp;</th><th>{$ax['grp_name']}</th><th>{$ax['grp_privs']}</th><th>{$ax['grp_categories']}</th><th>{$ax['grp_upload_files']}</th><th>{$ax['grp_rep_events']}</th><th>{$ax['grp_m-d_events']}</th><th>{$ax['grp_priv_events']}</th><th></th></tr>\n";
	while ($row = $stH->fetch(PDO::FETCH_ASSOC)) {
		...
		echo "</td>
		<td>".($row['upload'] < 1 ? $ax['no'] : $ax['yes'])."</td>
		<td>".($row['rEvents'] < 1 ? $ax['no'] : $ax['yes'])."</td>
		<td>".($row['mEvents'] < 1 ? $ax['no'] : $ax['yes'])."</td>
		<td>".($row['pEvents'] < 1 ? $ax['no'] : $ax['yes'])."</td>";
...
function editGroup(&$group) {
	...
	echo "<table class='list'>\n";
	if ($mode != 'add') { echo "<tr><td>{$ax['id']}:</td><td>&nbsp;{$group['id']}</td></tr>\n"; }
	...
	$stH = dbQuery("SELECT `ID`,`name`,`noverlap` FROM `categories` WHERE `status` >= 0 ORDER BY `sequence`");
	...
	while ($row = $stH->fetch(PDO::FETCH_ASSOC)) {
		...
		echo "<input type='checkbox' id='cats{$row['ID']}' name='cats[]' value='{$row['ID']}' onclick=\"checkN('cats',this);\"{$checked}><label for='cats{$row['ID']}'>{$row['name']}".($row['noverlap'] ? " ({$ax['grp_no_overlap']})" : '')."</label><br>";
	}
	echo "</td></tr>\n";
	echo "<tr><td><label for='upload'>{$ax['grp_may_upload_files']}</label>:</td><td><input type='checkbox' name='upload' id='upload' value='1'".($group['upload'] ? " checked='checked'" : '').">";
	echo "<tr><td><label for='rEvts'>{$ax['grp_may_post_revents']}</label>:</td><td><input type='checkbox' name='rEvts' id='rEvts' value='1'".($group['rEvts'] ? " checked='checked'" : '').">";
	echo "<tr><td><label for='mEvts'>{$ax['grp_may_post_mevents']}</label>:</td><td><input type='checkbox' name='mEvts' id='mEvts' value='1'".($group['mEvts'] ? " checked='checked'" : '').">";
	echo "<tr><td><label for='pEvts'>{$ax['grp_may_post_pevents']}</label>:</td><td><input type='checkbox' name='pEvts' id='pEvts' value='1'".($group['pEvts'] ? " checked='checked'" : '').">";
...
function addGroup(&$group) { //add group
	...
	do {
		...
		$stH = stPrep("INSERT INTO `groups` (`name`,`privs`,`catIDs`,`upload`,`rEvents`,`mEvents`,`pEvents`,`color`) VALUES (?,?,?,?,?,?,?,?)");
		stExec($stH,array($group['name'],$group['privs'],implode(',',$group['catIDs']),$group['upload'],$group['rEvts'],$group['mEvts'],$group['pEvts'],$group['color']));
		$msg = $ax['grp_added'];
		$mode = '';
	} while (false);
	return $msg;
...
function updateGroup($group) { //update group
	...
	do {
		if ($group['color'] and !preg_match("/^#[0-9A-Fa-f]{6}$/", $group['color'])) { $msg = $ax['grp_invalid_color']; break; }
		if (!preg_match("/^[\w\s\._-]{2,}$/u", $group['name'])) { $msg = $ax['grp_name_invalid']; break; }
		$stH = stPrep("UPDATE `groups` SET `name` = ?,`privs` = ?,`catIDs` = ?,`upload` = ?,`rEvents` = ?,`mEvents` = ?,`pEvents` = ?,`color` = ? WHERE `ID` = ?");
		stExec($stH,array($group['name'],$group['privs'],implode(',',$group['catIDs']),$group['upload'],$group['rEvts'],$group['mEvts'],$group['pEvts'],$group['color'], $group['id']));
		$msg = $ax['grp_updated'];
		$mode = '';
	} while (false);
	return $msg;
