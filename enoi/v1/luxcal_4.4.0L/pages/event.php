...
if (empty($lcV) or
		...
		(!empty($catID) and !preg_match('%^\d+$%', $catID)) or
...
if ($mode == 'edit' and !$refresh) { //show/edit event
	...
	if ($eid) { //get attachments
		$stH = stPrep("SELECT `attach` FROM `events` WHERE `ID` = ?");
		stExec($stH,array($eid));
...
if ($mode == "add_exe" or $mode == "upd_exe") {
	...
	do {
		...
		} elseif ($sDate < date("Y-m-d",time() + 86400 * $not)) { //$not >= 0
			$wMsg .= $xx['evt_not_in_past']."<br>";
		}
		if ($not == 0 and $sDate == $todayD) { $not = -1; $non = 1; } //notify now
		if (($not >= 0 or $non) and strlen($nml) < 5) {
		...
		if (!empty($_FILES['upload'])) {
			foreach($_FILES['upload']['name'] as $k => $fName) {
				if ($fName) {
					if ($_FILES['upload']['error'][$k]) { $eMsg .= "{$fName}: {$xx['evt_error_file_upload']}<br>"; continue; }
					if (!stripos($attType,substr($fName,-4))) { $eMsg .= "{$fName}: {$xx['evt_no_pdf_img_vid']}<br>"; continue; }
					if ($_FILES['upload']['size'][$k] > 4200000) { $eMsg .= "{$fName}: {$xx['evt_upload_too_large']}<br>"; continue; }
					$tsfName = date('YmdHis').$fName; //get timestamped file name
					move_uploaded_file($_FILES['upload']['tmp_name'][$k],"./attachments/{$tsfName}");
		...
		//notify now
		if ($non) {
			$evtArr = array ('cnm' => $cnm, 'ven' => $ven, 'des' => $desHtml, 'xf1' => $xf1Html, 'xf2' => $xf2Html, 'att' => $att); //Html: with hyperlinks
		...
		if ($mode == "add_exe") { //add new event
			$q = "INSERT INTO `events` (`private`,`title`,`venue`,`text1`,`text2`,`text3`,`attach`,`catID`,`userID`,`approved`,`notify`,`notMail`,`sDate`,`eDate`,`sTime`,`eTime`,`rType`,`rInterval`,`rPeriod`,`rMonth`,`rUntil`,`aDateTime`) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
			$stH = stPrep($q); //add to events table
			$result = stExec($stH,array($pri,$tit,$ven,$desHtml,$xf1Html,$xf2Html,$att,$cid,$uid,$apd,$not,$nml,$sDate,$eDate,$sTime,$eTime,$r_t,$r_i,$r_p,$r_m,$runtil,$todayDT));
			$stH = null;
			...
			if ($editN != 1) { //update the series
				$stH = stPrep("UPDATE `events`
					SET `private`=?, `title`=?, `venue`=?, `text1`=?, `text2`=?, `text3`=?, `attach`=?, `catID`=?, `userID`=?, `editor`=?, `approved`=?, `notify`=?, `notMail`=?, `sDate`=?, `eDate`=?, `sTime`=?, `eTime`=?, `rType`=?, `rInterval`=?, `rPeriod`=?, `rMonth`=?, `rUntil`=?, `mDateTime`=?
					WHERE `ID`=?"); //update events table
				stExec($stH,array($pri,$tit,$ven,$desHtml,$xf1Html,$xf2Html,$att,$cid,$uid,$uname,$apd,$not,$nml,$sDate,$eDate,$sTime,$eTime,$r_t,$r_i,$r_p,$r_m,$runtil,$modDT,$eid));
				...
				$stH = stPrep("INSERT INTO `events` (`private`,`title`,`venue`,`text1`,`text2`,`text3`,`attach`,`catID`,`userID`,`editor`,`approved`,`notify`,`notMail`,`sDate`,`eDate`,`sTime`,`eTime`,`rType`,`rInterval`,`rPeriod`,`rMonth`,`rUntil`,`aDateTime`,`mDateTime`) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)"); //add new event
				stExec($stH,array($pri,$tit,$ven,$desHtml,$xf1Html,$xf2Html,$att,$cid,$uid,$uname,$apd,$not,$nml,$sDate,$eDate,$sTime,$eTime,$r_t,$r_i,$r_p,$r_m,$runtil,$adt,$todayDT,));
