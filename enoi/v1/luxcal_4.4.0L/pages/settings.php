...
function buttonsValid($buttons,$range) {
	...
	foreach (str_split($buttons) as $buttonNr) {
		if (substr_count($range,$buttonNr) != 1) { return false; }
	}
	return true;
}
...
$errors = array_fill(0, 37, ''); $i = 0; //init
...
if (isset($_POST["save"])) { //validate settings
	...
	if (!buttonsValid($pSet['viewButtons'],' 123456789')) { $errors[$i] = " class='inputError'"; } $i++;
	if ($pSet['cookieExp'] < 1 or $pSet['cookieExp'] > 365) { $errors[$i] = " class='inputError'"; } $i++;
	if (!fieldsValid($pSet['evtTemplGen'],' 12345678')) { $errors[$i] = " class='inputError'"; } $i++;
	if (!fieldsValid($pSet['evtTemplUpc'],' 12345678')) { $errors[$i] = " class='inputError'"; } $i++;
	if (!fieldsValid($pSet['evtTemplPop'],' 12345678')) { $errors[$i] = " class='inputError'"; } $i++;
	if ($pSet['yearStart'] < 0 or $pSet['yearStart'] > 12) { $errors[$i] = " class='inputError'"; } $i++;
	if ($pSet['YvRowsToShow'] < 1 or $pSet['YvRowsToShow'] > 10) { $errors[$i] = " class='inputError'"; } $i++;
	if ($pSet['YvColsToShow'] < 1 or $pSet['YvColsToShow'] > 6) { $errors[$i] = " class='inputError'"; } $i++;
	if ($pSet['MvWeeksToShow'] < 0 or $pSet['MvWeeksToShow'] > 20) { $errors[$i] = " class='inputError'"; } $i++;
	if (!preg_match("/^[1-7]{1,7}$/", $pSet['workWeekDays'])) { $errors[$i] = " class='inputError'"; } $i++;
	if ($pSet['XvWeeksToShow'] < 4 or $pSet['XvWeeksToShow'] > 20) { $errors[$i] = " class='inputError'"; } $i++;
...
//the following regexs use lookahead assertion
	...
	if ($cronHost == 2 and !preg_match('%(?:(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.){4}%',$cronIpAd.'.')) { $errors[$i] = " class='inputError'"; } $i++;
	...
	if (!fieldsValid($pSet['popFieldsMcal'],' 1234567')) { $errors[$i] = " class='inputError'"; } $i++;
	if (!fieldsValid($pSet['popFieldsSbar'],' 1234567')) { $errors[$i] = " class='inputError'"; } $i++;
...
echo "<fieldset><legend>{$ax['set_navbar_settings']}</legend>
	...
	<input type='checkbox' id='viewm' name='pSet[viewMenu]' value='1'".($pSet['viewMenu'] == 1 ? " checked='checked'" : '')."><label for='viewm'>{$ax['viewMenu_label']}</label>&nbsp;&nbsp;
	...
	<div class='sLabel' onmouseover=\"pop(this,'".htmlspecialchars($ax['viewButtons_text'])."', 'normal')\">{$ax['viewButtons_label']}:</div>
	<div class='sValue'><input type='text'{$errors[$i++]} name='pSet[viewButtons]' maxlength='8' size='7' value=\"{$pSet['viewButtons']}\"></div><br>
	...
	<option value='9'".($pSet['defaultView'] == "9" ? $selected : '').">{$xx['hdr_matrix']}</option>
	</select></div><br>
...
echo "<fieldset><legend>{$ax['set_event_settings']}</legend>
	...
	<div class='sValue'>{$ax['xField_label']}: <input type='text' name='pSet[xField1Label]' maxlength='15' size='12' value=\"{$pSet['xField1Label']}\">&nbsp;&nbsp;&nbsp;<input type='checkbox' id='xF1pub' name='pSet[xField1Public]' value='1'".($pSet['xField1Public'] == 1 ? " checked='checked'" : '')."><label for='xF1pub'>{$ax['xField_public']}</label></div><br>
...
echo "<fieldset><legend>{$ax['set_user_settings']}</legend>
	...
	<div class='sValue'><input type='checkbox' name='pSet[selfReg]' value='1'".($pSet['selfReg'] == 1 ? " checked='checked'" : '').">&nbsp;&nbsp;<select name='pSet[selfRegGrp]'>\n";
	...
	echo "</select>&nbsp;{$ax['user_group']}</div><br>
...
echo "<fieldset><legend>{$ax['set_view_settings']}</legend>
	...
	<div class='sLabel' onmouseover=\"pop(this,'".htmlspecialchars($ax['evtTemplate_text'].'<br>'.$ax['templFields_text'])."', 'normal')\">{$ax['evtTemplate_label']}:</div>
	<div class='sValue'><input type='text'{$errors[$i++]} name='pSet[evtTemplGen]' maxlength='8' size='6' value=\"{$pSet['evtTemplGen']}\">&nbsp;{$ax['evtTemplGen']}&nbsp;&nbsp;&nbsp;<input type='text'{$errors[$i++]} name='pSet[evtTemplUpc]' maxlength='8' size='6' value=\"{$pSet['evtTemplUpc']}\">&nbsp;{$ax['evtTemplUpc']}&nbsp;&nbsp;&nbsp;
	<input type='text'{$errors[$i++]} name='pSet[evtTemplPop]' maxlength='8' size='6' value=\"{$pSet['evtTemplPop']}\">&nbsp;{$ax['evtTemplPop']}</div><br>
	...
	<div class='sLabel' onmouseover=\"pop(this,'".htmlspecialchars($ax['YvRowsColumns_text'])."', 'normal')\">{$ax['YvRowsColumns_label']}:</div>
	<div class='sValue'><input type='text'{$errors[$i++]} name='pSet[YvRowsToShow]' maxlength='2' size='2' value='{$pSet['YvRowsToShow']}'> {$ax['rows']} (1 - 10)&nbsp;&nbsp;&nbsp;<input type='text'{$errors[$i++]} name='pSet[YvColsToShow]' maxlength='1' size='2' value='{$pSet['YvColsToShow']}'> {$ax['columns']} (1 - 6)</div><br>

	<div class='sLabel' onmouseover=\"pop(this,'".htmlspecialchars($ax['MvWeeksToShow_text'])."', 'normal')\">{$ax['MvWeeksToShow_label']}:</div>
	<div class='sValue'><input type='text'{$errors[$i++]} name='pSet[MvWeeksToShow]' maxlength='2' size='2' value='{$pSet['MvWeeksToShow']}'> (2 - 20 {$ax['or']} 0 {$ax['or']} 1)</div><br>
	...
	<div class='sLabel' onmouseover=\"pop(this,'".htmlspecialchars($ax['XvWeeksToShow_text'])."', 'normal')\">{$ax['XvWeeksToShow_label']}:</div>
	<div class='sValue'><input type='text'{$errors[$i++]} name='pSet[XvWeeksToShow]' maxlength='2' size='2' value='{$pSet['XvWeeksToShow']}'> (4 - 20)</div><br>
	...
	<div class='sLabel' onmouseover=\"pop(this,'".htmlspecialchars($ax['dwStartEndHour_text'])."', 'normal')\">{$ax['dwStartEndHour_label']}:</div>
	<div class='sValue'><input type='text'{$errors[$i++]} name='pSet[dwStartHour]' maxlength='2' size='2' value='{$pSet['dwStartHour']}'> (0 - 18)&nbsp;&nbsp;
	<input type='text'{$errors[$i++]} name='pSet[dwEndHour]' maxlength='2' size='2' value='{$pSet['dwEndHour']}'> (6 - 24)</div><br>
...
echo "<fieldset><legend>{$ax['set_perfun_settings']}</legend>
	...
	<div class='sLabel' onmouseover=\"pop(this,'".htmlspecialchars($ax['cronHost_text'])."', 'normal')\">{$ax['cronHost_label']}:</div>
	<div class='sValue'><input type='radio' id='cron0' name='cronHost' value='0'".($cronHost == 0 ? " checked='checked'" : '')."><label for='cron0'>{$ax['local']}</label>&nbsp;&nbsp;&nbsp;
	<input type='radio' id='cron1' name='cronHost' value='1'".($cronHost == 1 ? " checked='checked'" : '')."><label for='cron1'>{$ax['remote']}</label>&nbsp;&nbsp;&nbsp;
	<input type='radio' id='cron2' name='cronHost' value='2'".($cronHost == 2 ? " checked='checked'" : '')."><label for='cron2'>{$ax['ip_address']}</label>: <input type='text'{$errors[$i++]} name='cronIpAd' maxlength='15' size='12' value=\"{$cronIpAd}\"></div><br>
