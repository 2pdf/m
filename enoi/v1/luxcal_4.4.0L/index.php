...
if (!empty($_COOKIE)) { $_COOKIE = array_map('stripslashes', $_COOKIE); }
...
$stH = stPrep("SELECT u.`ID`,u.`name`,u.`email`,g.`privs`,g.`catIDs`,g.`rEvents`,g.`mEvents`,g.`pEvents`,g.`upload` FROM `users` AS u INNER JOIN `groups` AS g ON g.`ID` = u.`groupID` WHERE u.`ID` = 1 OR u.`ID` = ? ORDER BY u.`ID` DESC"); //if userID not found, revert to public user
stExec($stH,array($_SESSION['uid']));
list($uid,$uname,$umail,$privs,$ucats,$revts,$mevts,$pevts,$uplod) = $stH->fetch(PDO::FETCH_NUM); //user & group data
$row = $stH->fetch(PDO::FETCH_NUM);
...
$pages = array (
	 ...
	 '9' => array ('views/matrix.php','f','0','m','f','0','','y',''),
