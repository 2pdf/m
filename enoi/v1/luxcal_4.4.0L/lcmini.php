...
if (isset($_REQUEST['cG'])) { $_SESSION['cG'] = $_REQUEST['cG']; } //set user group filter
if (isset($_REQUEST['cC'])) { $_SESSION['cC'] = $_REQUEST['cC']; } //set category filter
$stH = dbQuery("SELECT g.`privs`, g.`catIDs` FROM `users` AS u INNER JOIN `groups` AS g ON g.`ID` = u.`groupID` WHERE u.`ID` = 1");
$row = $stH->fetch(PDO::FETCH_ASSOC);
$stH = null; //release statement handle!
$privs = $row['privs'];
if (empty($_SESSION['cC'])) { $_SESSION['cC'] = explode(',',$row['catIDs']); } //group categories
...
retrieve($sDate,$eDate,'gc'); //retrieve events

/* display header */
$fullCalUrl = $set['mCalUrlFull'] ? $set['mCalUrlFull'] : 'index.php?lc&amp;cP=2&amp;cD='.$dateD1.'&amp;cC%5B%5D='.implode('&amp;cC%5B%5D=',$_SESSION['cC']);
...
echo "<tr>\n<th colspan='7' class='endBar'>
	...
	<span class='floatR'><a class='endBarTxt' href='http://www.luxsoft.eu' target='_blank'><span class='footLB'><span title='V".LCV."'>L</span>ux</span><span class='footLR'>Soft</span></a></span>\n";
