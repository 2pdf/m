...
function showEvents(&$events,$date) {
	...
	foreach ($events as $evt) {
		...
		if ($hoverBox) {
			$popText = "<b>{$evtTime}:".((!$evt['mde'] and $evt['eti']) ? ' - '.ITtoDT($evt['eti']) : '')." {$evt['tix']}</b><br>";
			...
			if ($showDetails or $evt['mayE']) {
				echo "<h6>{$chBox}<span class='point'{$eStyle} onclick=\"editE({$evt['eid']},'{$date}');\"{$popAttr}>{$evt['tix']}</span></h6>\n";
				echo makeE($evt,$set['evtTemplUpc'],'br',"<br>")."\n";
			} else {
				echo "<h6>{$chBox}<span{$eStyle}{$popAttr}>{$evt['tix']}</span></h6>\n";
			}
			echo "</td></tr></table>\n";
			$evtDone[] = $evt['eid'];
...
$evtList = array();
$hoverBox = false;
if ($set['popBoxUpc'] and $popFieldsArr = str_split($set['evtTemplPop'])) {
