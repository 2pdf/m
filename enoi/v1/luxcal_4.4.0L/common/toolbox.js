...
function newEx(dt,cid){ //new event - date / cat ID
	var date = (arguments[0]) ? '&evD='+arguments[0] : '';
	var catID = (arguments[1]) ? '&catID='+arguments[1] : '';
	var evtWinL = (screen.width-evtWinW)/2;
	var evtWin = window.open('index.php?lc&xP=10&mode=add'+date+catID,'evtWin','menubar=no,location=no,toolbar=no,height=' + evtWinH +',width=' + evtWinW + ',top=' + evtWinT + ',left=' + evtWinL + ', scrollbars=0');
	evtWin.focus();
	return false;
}
...
function checkE(obj,evtID,evtDt) { //ajax - toggle the event check mark
	var xhttp;
	...
	xhttp.open("POST","common/checkevt.php",true);
	xhttp.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
	xhttp.send("evtID="+evtID+"&evtDt="+evtDt);
}

function detach(obj,evtID,fName) { //ajax - detach file from event
	var xhttp;
	...
	xhttp.open("POST","common/detach.php",true);
	xhttp.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
	xhttp.send("evtID="+evtID+"&fName="+fName);
}
...
function popM(pElm,popContent,popClass,popMaxChar){
	var offsetX = -60, offsetY = 16; //x, y offset of box
	var maxLineLen = 80; //default max line length in box
	var popobj;
//	var parX = pElm.width;
	var parX = pElm.offsetLeft - pElm.scrollLeft + pElm.clientLeft;
	var parY = pElm.offsetTop - pElm.scrollTop + pElm.clientTop;
	...
	popobj.style.left = parX-20+"px";
	popobj.style.top = parX+10+"px";
	return;
}
