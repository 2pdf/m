...
function optButton() {
	global $xx, $set, $dbSel, $privs;

	if (!($dbSel and $privs == 9) and !$set['viewMenu'] and !$set['groupMenu'] and !$set['userMenu'] and !$set['catMenu'] and !$set['langMenu']) { //no menus
		echo '&nbsp;'; //dummy
	} else {
...
function viewButtons() {
	...
	if ($set['viewButtons']) {
		$xxIndex = array('','year','month','month','week','week','day','upcoming','changes','matrix');
		foreach (str_split($set['viewButtons']) as $viewNr) {
			$label = $xx["hdr_{$xxIndex[$viewNr]}"];
			echo "<button type='button' title=\"{$xx['hdr_go_to_view']}\" onclick=\"window.location.href='index.php?lc&amp;cP={$viewNr}'\">{$label}</button>\n";
		}
	}
}
...
function optPanel() { //make options panel
	global $xx, $set, $privs, $ucats, $pcats, $cP, $xU, $dbDir, $dbSel;
		...
		if (!($dbSel and $privs == 9) and !$set['viewMenu'] and !$set['groupMenu'] and !$set['userMenu'] and !$set['catMenu'] and !$set['langMenu']) { return; } //no menus
...
<input type='checkbox' id='cP8' name='cP' value='8' onclick=\"check1('cP',this);\"{$checkedA[8]}><label for='cP8'>{$xx['hdr_changes']}</label><br>
<input type='checkbox' id='cP9' name='cP' value='9' onclick=\"check1('cP',this);\"{$checkedA[9]}><label for='cP9'>{$xx['hdr_matrix']}</label>
</div>\n</div>\n";
	}
	if ($set['groupMenu']) {
...
<script>window.onpageshow = function() {if((window.location != window.parent.location)) xshow('urlButton',false);}</script>
</head>\n
<body>\n
TXT;
		headerHdr();
...
TXT;
		...
		echo "<div class='floatR'>\n";
		calButton();
		if (!$_SESSION['mobile']) { prtButton(); }
		admMenu();
		if (!$_SESSION['mobile']) { hlpButton(); }
		logButton();
		echo "</div>\n";
		optButton();
		echo "</div>\n";
