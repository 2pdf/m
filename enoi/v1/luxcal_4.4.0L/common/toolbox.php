...
$rxURL = '~(https?://)((?:\w[\w-]+\.)+[a-z]{2,15}(?:[/?#:][^<>\s\[]*[^<>\s\[\.])?)(?:\s*\[([^<>\[]+?)\]|\s|<|\.|\)|$)~im'; //http://www.mysite.xxx [title]
...
$rxIMGTags = '~<img.*?/([\w\d]+/[\w\d]+\.(?:gif|jpg|png)).*?>~i'; // <img src=...>

$rxIMG = '~(?:^|\s)(([^\s/.]*/)*([^\s/.]+\.(?:gif|jpg|png)))(?:\.|\s|$)~i'; //thumbnails/mynail.gif (or .jpg, or .png)

$attType = '|.pdf|.jpg|.gif|.png|.ogg|.mp4|.webm|.wma|.wav|.avi|.mpg|.mov|.wmv'; //attachement file types
	...
	if ($type == 'br') { //type br
		foreach (str_split($template) as $fieldNr) {
			...
			case '7':
				...
				break;
			case '8':
//				if ($evt['att']) { $eArray[] = $xx['evt_attachments'].': '.trim(preg_replace('%;\d{14}%',', ',$evt['att']),', '); }
				if ($evt['att']) {
					$attachments = array();
					foreach(explode(';',trim($evt['att'],';')) as $attachment) {
						$attachments[] = "<a href='".calRootUrl()."attachments/{$attachment}' target='_blank'>".substr($attachment,14)."</a>";
					}
		...
		foreach (str_split($template) as $fieldNr) {
			...
			case '8':
				if ($evt['att']) {
					...
					foreach(explode(';',trim($evt['att'],';')) as $attachment) {
						$eArray[] = "<tr><td>{$label}</td><td><a href='".calRootUrl()."attachments/{$attachment}' target='_blank'>".substr($attachment,14)."</a></td></tr>";
						$label = '';
					}
				}
			}
...
//make calendar root URL
function calRootUrl() {
	global $set;

	$qmPos = strpos($set['calendarUrl'],'?');
	$rootUrl = $qmPos === false ? $set['calendarUrl'] : substr($set['calendarUrl'],0,$qmPos);
	rtrim($rootUrl,'/').'/';
...
function addUrlImgTags($html,$ui='xx') {
	...
	if ($ui[0] == 'x') { //create URL links
		$html = preg_replace_callback($rxURL,function ($m) { return " <a class='link' href='".$m[1].$m[2]."' target='_blank'>".(!empty($m[3])?$m[3]:$m[2])."</a>"; },$html);
	}
	if ($ui[1] == 'x') { //create image links
		$html = preg_replace_callback($rxIMG,function ($m) { return " <img class='thNail' src='".calBaseUrl().(!empty($m[2])?$m[2]:'thumbnails/')."{$m[3]}' alt='{$m[3]}'>"; },$html);
	}
	return $html;
}
