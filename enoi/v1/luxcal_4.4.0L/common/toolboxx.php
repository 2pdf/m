...
$defSet = array( //name => default value, outline
	...
	'viewButtons' => array('','View buttons to be displayed on the navbar (1:year, 2:month, .... 8:changes)'),
	'defaultView' => array(2,'Calendar view at start-up (1:year, 2:month, .... 8:changes)'),
...
function initGroups() { //init groups table
	$groups = array(array(1,'No access',0,'0',0),array(2,'Admin',9,'0',1),array(3,'Read access',1,'0',0),array(4,'Post Own',2,'0',0),array(5,'Post All',3,'0',0),array(6,'Manager',4,'0',1));
	dbTransaction('begin');
	$stH = stPrep("REPLACE INTO `groups` (`ID`,`name`,`privs`,`catIDs`,`upload`) VALUES (?,?,?,?,?)");
...
function importSqlFile(&$sqlArray) { //Import a database backup file
	...
	$patTable = '~\s+["`]?(?:\w{1,20})?_?(events|categories|users|groups|settings)["`]?(\s*[(;V])~'; //strip calID_ and replace " by ` around table names
	$repTable = ' `$1`$2';
	...
	do {
		...
		if (!$stH) { //column 'attach' not present - version 4.3
			...
			dbQuery("INSERT INTO `eventsNew` (`ID`, `private`, `title`, `venue`, `text1`, `text2`, `text3`, `catID`, `userID`, `editor`, `approved`, `checked`, `notify`, `notMail`, `sDate`, `eDate`, `xDates`, `sTime`, `eTime`, `rType`, `rInterval`, `rPeriod`, `rMonth`, `rUntil`, `aDateTime`, `mDateTime`, `status`) 
				SELECT `ID`, `private`, `title`, `venue`, `text1`, `text2`, `text3`, `catID`, `userID`, `editor`, `approved`, `checked`, `notify`, `notMail`, `sDate`, `eDate`, `xDates`, `sTime`, `eTime`, `rType`, `rInterval`, `rPeriod`, `rMonth`, `rUntil`, `aDateTime`, `mDateTime`, `status` FROM `events`"); //copy content events table
			dbQuery("INSERT INTO `usersNew` SELECT * FROM `users`"); //copy content users table
			dbQuery("INSERT INTO `groupsNew` (`ID`, `name`, `privs`, `catIDs`, `rEvents`, `mEvents`, `pEvents`, `color`, `status`) 
				SELECT `ID`, `name`, `privs`, `catIDs`, `rEvents`, `mEvents`, `pEvents`, `color`, `status` FROM `groups`"); //copy content groups table
			dbQuery("INSERT INTO `categoriesNew`  SELECT * FROM `categories`"); //copy content categories table
			dbQuery("INSERT INTO `settingsNew` SELECT * FROM `settings`"); //copy content settings table
			break; //done
		}
