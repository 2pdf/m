...
Input params:
- start date (in 2016-04-28 format)
...
function grabChanges($sDate,$allEvents) { //query db for changes since $sDate
	global $set, $privs, $ucats, $pcats, $evtList;
	...
	$filter = '';
	if (!$allEvents) {
		if (count($_SESSION['cG']) > 0 and $_SESSION['cG'][0] != 0) {
	...
	WHERE ".($catList != '0' ? "c.`ID` IN ($catList) AND " : '')."((SUBSTR(e.`aDateTime`,1,10) != '9999-00-00' AND SUBSTR(e.`aDateTime`,1,10) >= '$sDate') OR (SUBSTR(e.`mDateTime`,1,10) != '9999-00-00' AND SUBSTR(e.`mDateTime`,1,10) >= '$sDate'))".$filter;
	$stH = dbQuery($q);
